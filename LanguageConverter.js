...
function LanguageConverter() {
  ...
  const handleTranslate = async () => {
    ...
    if (data.detected_source) {
      const langName = languages[data.detected_source];
      alert(`Detected Source Language: ${langName} (${data.detected_source})`);
    }
  };

  const handleVoiceInput = async () => {
  fadeOutHomeSound();           // 🔇 Fade out when speaking
  ...
  try {
    ...
    resumeHomeSound();         // 🔊 Resume after 10s
  }
};
  ...
  return (
    <div style={{ background: '#f4f4f4', minHeight: '100vh', padding: '40px 20px' }}>
      <div style={{
        ...
        <button onClick={handleVoiceInput} style={{ ...buttonStyle, marginBottom: '20px' }}>
          🎙 Speak
        </button>
